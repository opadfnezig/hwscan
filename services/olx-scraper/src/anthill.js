...
/**
 * Fetch JSON API via anthill. Parses body as JSON.
 */
export async function scrapeJSON(url, opts = {}) {
  const data = await scrape(url, opts);
  if (data.statusCode === 404) return { _notFound: true, statusCode: 404 };
  if (data.statusCode === 410) return { _notFound: true, statusCode: 410 };
